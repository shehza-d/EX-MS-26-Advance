...
const App = () => {
  ...
  return (
    <>
      <Grid className="container" container spacing={2} sx={{marginTop: '30px'}}>
        <Grid size={{ xs: 12, md: 6, lg: 4 , xl: 3 }}>
          <Typography sx={{border: '1px solid black'}} variant='h4'>size=4</Typography>
        </Grid>
        <Grid size={{ xs: 12, md: 6, lg: 4 , xl: 3 }}>
          <Typography sx={{border: '1px solid black'}} variant='h4'>size=4</Typography>
        </Grid>
      ...
      <Typography variant='h3' align='center'>Our Products</Typography>
      {loading && <Box sx={{
        display: 'flex',
      ...
      <Box className="d-flex justify-content-center flex-wrap gap-5">
        {products && products.map(item => {
          return <MediaCard key={item.id} title={item.title} description={item.description} image={item.thumbnail} />
        })}
      </Box>
    </>
  )
